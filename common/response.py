"""
Standard error code framework for API responses like below in case of errors- 
{
    "code" : "",
    "message" : ""
}
...
def get_standard_response(error: BaseErrorCode, response: dict) -> Response:
    response_body = (
        error.to_json()
        if error.code
        not in [BaseErrorCode.SUCCESS, BaseErrorCode.CREATED, BaseErrorCode.NO_CONTENT]
        else response
    )
...
def get_paginated_response(
    ...
    if error.code not in [
        ...
        BaseErrorCode.CREATED,
        BaseErrorCode.NO_CONTENT,
    ]:
        ...
        return instance.get_paginated_response(
            serializer_class(paginated_queryset, many=True).data
        )
